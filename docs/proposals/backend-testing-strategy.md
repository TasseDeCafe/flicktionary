...
4. **Test the enrichment-worker driver** (`service/long-running/enrichment-worker`) once (1) lands. Now unblocked. Note for the implementation: the worker only exposes `initialize()` (a 2s `setInterval` poll) — a clean, fast test wants a `tickOnce()` exposed on the interface (or the internal `tick` exported) rather than real-time polling waits; that testability tweak should land with the test.
