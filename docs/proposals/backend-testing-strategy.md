...
Counts (2026-07-14): 49 `*.unit.test.ts` files, 30 `*.integration.test.ts` files in `apps/backend/src`.

**Unit tests** are in good shape. The dominant style extracts pure functions and feeds them hand-built row objects (`service/practice/*.unit.test.ts`, `service/lesson-import/extract-lesson-job.unit.test.ts`) — fast, no mocking framework. The LLM passes are unit-tested at the parser level against static strings (`transport/third-party/anthropic/passes/*.unit.test.ts`). Only 6 files use `vi.mock` module mocking. They originally clustered around one cause — the Anthropic client was a module singleton (`getAnthropicClient()` in `transport/third-party/anthropic/anthropic-client.ts`) with no injection point, so services that call the LLM had to patch modules. That cause is fixed (plan item 1): LLM calls now inject through `AnthropicPassesInterface`, and the surviving `vi.mock`s only patch internal service seams (language-mode, wiktionary-grounding, apply-study-intent, sentry).
...
**Gap (largely closed by plan item 2):** the core Flicktionary oRPC surfaces now have golden-path integration tests driving `buildApp` over HTTP — glosses, study-sessions (importText + list/get), highlights (create/list/delete + text-segments listing rides along), cards (adhoc create/get/edit/remove), practice (rate → due-summary → undo), lesson-import (batch create/resume/poll + profiles). Still untested over HTTP: card-chat, ghosts, chunks, content-sources, text-tracks — add one when its surface next changes, not as a sweep. `service/long-running/` coverage: the enrichment-worker driver now has an integration test (plan item 4); telegram-polling-worker and subscription-cache remain untested.
...
1. **Make the Anthropic client injectable** with a scripted fake (canned per-pass outputs). Kills most `vi.mock` usage; unblocks everything below. Inject TMDB/OpenSubtitles the same way when their code first needs a test, not preemptively. **Implemented** — the seam is the pass layer, not the raw client: `AnthropicPassesInterface` (`transport/third-party/anthropic/anthropic-passes.ts`) bundles the 12 pass functions plus `createChatCompletion` (card chat builds its bespoke prompt in the service; only the raw completion call crosses the seam). Injecting at pass granularity means tests script typed domain outputs instead of authoring Anthropic wire-format fixtures — the same reason HTTP interception was rejected for Stripe above; the pass modules' prompts + parsers stay unit-tested directly against static strings. `AnthropicPasses()` is the real implementation (still backed by the `getAnthropicClient()` singleton internally), `MockAnthropicPasses(overrides)` is the scripted fake whose unscripted methods throw loudly. It rides `AppDependencies` (buildApp default: real) and `ProcessingDependencies` (covers the whole worker side), and subsumed the ad-hoc `detectLanguage?` injection points in import-text / telegram-bot. Anthropic module mocks (`vi.mock` of `anthropic-client` / `passes/*`) are now zero; the 6 remaining `vi.mock` files only patch internal service seams (language-mode, wiktionary-grounding, apply-study-intent, sentry) — out of scope here.
2. **Add ~6–10 golden-path oRPC integration tests**: practice (compose queue → rate → resume), cards/review editing, glosses, highlights, lesson-import, study-sessions. Golden path + one auth failure each; not exhaustive scenarios. Written with per-test unique users from day one (no global truncation) so they never join the serialization problem. **Implemented** — six router test files, all following the `glosses-router.integration.test.ts` pattern (supertest through `buildApp({ anthropicPasses: MockAnthropicPasses({...}) })`, golden path + 401 + one domain failure): glosses (fast-gloss + language detection), study-sessions (importText golden path, content-hash idempotency, 422 MISSING_CEFR), highlights (create with enrich-job enqueue asserted via processing-status, list, delete, cross-user 404 — resolves segment ids through the text-segments route so that wiring is covered too), cards (adhoc create with scripted `basicDataPass` → get → updateFields → removeFromSession, 400 cefr_not_set), practice (rate a fresh keep as introduction → due-summary deltas → undo restores, 400 illegal pool/skill), lesson-import (createBatch/resume/poll — LLM-free by design, the worker owns extraction — profiles upsert/list, 409 confirm-while-extracting). Seeding convention: tests seed through the API where a synchronous flow exists (`/cards/adhoc` is how practice gets a kept term) and through repos only for prefs (`setNativeLanguage`, `upsertCefr`).
...
4. **Test the enrichment-worker driver** (`service/long-running/enrichment-worker`) once (1) lands. **Implemented** — `tickOnce()` (the internal `tick`, exposed on `EnrichmentWorkerInterface`; no-op on the mock) replaces real-time polling waits. `enrichment-worker.integration.test.ts` drives one real `extract_lesson` job through claim → dispatch → markDone with a scripted `extractLessonPass` and asserts the batch flips to 'ready' with planned rows. One deliberate fake: the test injects a `claimBatch` scoped to its own batch (same lease/attempts SQL shape) because the real claim is global and the shared test DB accumulates other tests' pending jobs — an unscoped tick would claim them and fail them against the unscripted mock, corrupting concurrent tests. The real claim SQL keeps its coverage in the repository's own tests.
