...
- **Web surfaces**:
  ...
  - **Dashboard featured section** ("Start with shared content", between the Getting-started checklist and the Daily Mix) — featured entries only, same chip row (local state, no URL), up to 4 cards + "Explore all →"; cards navigate to the same detail screen. Renders only once the sessions query has resolved AND the user has fewer than 3 sessions, and only when featured entries exist; it retires itself as the library grows. Removing a session invalidates the catalog feed too — a removal auto-unshares its entry server-side, and the dashboard hosts both surfaces at once.
