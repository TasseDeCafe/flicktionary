...
Three source kinds in the MVP, all feeding the same `text_segment` table. (A fourth ingestion path — **YouTube via the companion browser extension** — also lands in `text_segment` but is not a web-app flow; see `SPEC.md` → "Browser extension (companion)".)
