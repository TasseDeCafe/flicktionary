...
- **Movie & TV subtitles.** Movies and TV episodes share one wizard flow; the user picks `Movie` or `TV show` after choosing the study language. TV adds a season → episode selection (TMDB `/search/tv` → `/tv/{id}` seasons, season 0 / Specials hidden → `/tv/{id}/season/{n}` episodes). Each episode is its own `content_source(type='tv')` titled `"<show> · S0xE0y · <episode>"`, deduped globally on `(tmdbShowId, seasonNumber, episodeNumber)` (a DB partial unique index backs the app-level check); movies stay `type='movie'`.
  ...
  - On import, SRTs are normalized into per-line `text_segment` records (with `start_ms`/`end_ms`). The raw SRT is not preserved as a single blob.
  - **OpenSubtitles imports dedupe before downloading.** All users share one OpenSubtitles daily download quota (single API key, no per-user login), so a `file_id` already imported for the same content source + language is answered from the existing track — matched on `text_tracks.external_id` — without calling `/download`; the post-parse hash dedupe only covers byte-different SRTs from other entry points. Upstream limits surface as 429 instead of a generic 500: OpenSubtitles/TMDB 429s and the OpenSubtitles 406 spent-daily-quota signal map to `TOO_MANY_REQUESTS` with code `UPSTREAM_RATE_LIMITED` (transient) or `UPSTREAM_QUOTA_EXCEEDED` (retrying today won't help). The web app never retries 429s and shows a specific toast for the upstream codes ("try again tomorrow" for the spent quota) rather than the user-facing rate-limit overlay.
- **Pasted text.** First-class, not a fallback. The user pastes raw text (50–20,000 chars), provides a short title (auto-suggested from the first ~60 chars), and picks the language (auto-detected from the paste via a Haiku call as the user pauses typing; the manual override always wins).
  ...
  - Same dedup-hash plumbing as subtitles.
...
- **Content moderation at ingestion.** The five **user-authored** text paths — paste, manual `.srt` upload, extension article/selection import, Telegram, lesson-notes import — run a moderation gate before anything is stored (`moderateIngestText`, `apps/backend/src/service/moderation/moderate-ingest-text.ts`, over the `moderationPass` Haiku classifier). Platform subtitle ingestion (extension YouTube/streaming) and OpenSubtitles imports are deliberately excluded — `importFromOpenSubtitles` delegates into `importSrt`, so the moderation dep is injected only by the upload router. The full text is covered (≤20k-char chunks classified in parallel, worst verdict wins — no sampling blind spots); the title is included where the call site has it (extension/Telegram/lesson). Verdicts: `allow` / `flag <category>` / `block <category>`, with the hard-block policy enforced in the parser, not the prompt: only `sexual-explicit` and `csam` ever block (a `flag sexual-explicit` promotes to block; a `block violence` downgrades to flag). Blocked → the import is rejected with a 422 `CONTENT_BLOCKED` and an honest message (web surfaces a localized toast; the Telegram bot replies in-chat, terminal, no pending stash) — including on dedupe hits whose stored verdict is still null. Allowed verdicts persist on `text_tracks.moderation_status/_category` (written in the track INSERT; lesson batches store theirs on `import_batches` and copy it to the lesson track at confirm) and gate the content-sharing catalog (see **Shared content (Explore)** below — only an explicit `clean` may publish); flagged content is accepted for private study. A third status, `blocked`, exists only for tracks moderated *after* ingest (the share-time YouTube check): it permanently bars publishing while leaving private study untouched. Classifier failures **fail open** (import proceeds, null verdict, logged without message bodies), and a later re-import of a null-verdict document re-checks and backfills — first verdict wins, never overwritten.
...
- User content is private by default and RLS-scoped. The **one deliberate exception** is the shared-content catalog below: publishing exposes the source title **and the track's full segment text** to any authenticated user — the catalog's detail endpoint serves the text read-only before any session exists (so users can judge content before adding it, and admins can see what they moderate) — and lets other users create sessions on the track. Nothing else about a source ever leaves its owner — raw source URLs in particular are never public (an extension-imported URL is the verbatim tab URL and can carry signed tokens; only the server-parsed domain is exposed).

## Shared content (Explore)

Users' own content can be published into a public, cross-user catalog (`shared_content_entries`, one row per published text track) that new and anonymous users browse, preview in full, and add from. Backend: `apps/backend/src/service/shared-content/` (policy, publish service, add-to-library) + `shared-content-router`.

- **Share modes per source type** (`SHARE_MODE_BY_SOURCE_TYPE`): `youtube` and `article` are **auto** (published on successful ingest, no user action); `text` (web pastes, extension free-text selections, Telegram imports) is **opt-in** (the paste wizard's "Share to Explore" checkbox rides `studySessions.create`, or the owner's share toggle later); `movie`/`tv`/`streaming` are **never shareable** (copyright posture — privately fetching subtitles for your own study is one thing, a browsable catalog of them is another), and `lesson`/`book`/`adhoc` are excluded too. The per-type policy is the single guard; revisiting the movie/TV stance is a one-line change there.
- **Publish eligibility** (`publishIfEligible`, every path): type's mode allows the trigger, caller **owns the source** (`created_by_user_id`), caller is **not anonymous** (guests never share — authoritative `auth.users.is_anonymous` read), track moderation is an explicit `clean`, and the **title** passes its own moderation call (titles bypass the ingest gate: paste moderation covers segment text only, YouTube titles are client-sent). YouTube tracks — the one shareable surface without an ingest gate — are moderated at publish time over the full segment text and the verdict persisted (`clean`/`flagged`/`blocked`), so each track is checked at most once. All ineligibilities are quiet skips; ingest-path publishes are fire-and-forget so ingest latency is unchanged.
- **Identity & dedup**: every entry carries a `canonical_key` (`youtube:{videoId}`, else `hash:{track sha256}`). One **live** entry per key across all users (partial unique index); an admin tombstone on any copy blocks all future copies of that content, while a mere owner unshare frees the key for others.
- **Lifecycle**: "live" = neither `unshared_at` nor `removed_at` set. Owner opt-out is an **upsert** (the row is created already-unshared if a background publish hasn't landed yet, so the opt-out always wins the race). Tombstones (`removed_at` + required `removed_reason`, admin-only via `assertTestUser`-gated endpoints; the admin UI lives inline in Explore — see Web surfaces) are permanent — the owner's re-share toggle refuses. `adminList` takes an optional server-side `status` filter (`WHERE` before `LIMIT` — client-side filtering over the flat latest-200 would lose older non-live entries once the catalog outgrows the cap). Unshare/removal never touches sessions other users already created. Auto-unshare fires when the owner's session for the track is soft-deleted (the session is the only toggle surface) and on **account deletion** (before the auth row goes; recipients' copies deliberately survive — disclosed in the privacy policy). A **title-change fence** re-moderates the always-overwritten source title on re-ingest whenever the source has live entries and takes them all down if the new title isn't clean.
- **Consumption**: `sharedContent.list` (feed: live entries, featured-first then newest, ≤100; DTO = title/type/language/youtubeVideoId/sourceDomain plus `inLibrary` — a viewer-specific flag computed via an index-backed LEFT JOIN on the same `(user_id, text_track_id, target_language) WHERE deleted_at IS NULL` key `addToLibrary`'s find-or-create uses, so the sharer's own entry reads `true` and a soft-deleted session makes the entry addable again; the list only — detail/admin DTOs stay viewer-independent). `sharedContent.get` (`GET /shared-content/{entryId}/detail`) is the full-text preview behind a catalog card: the list DTO plus the track's complete segment text (`'\n'`-joined, index order), `segmentCount`, and a `status` — for normal users live entries only (unknown or non-live ids 404, `status` always `live`, `removedReason` null); admins (`isTestUserEmail`) also open unshared/removed entries and get the real status + removal reason, so moderation is never blind. Word count / reading stats are derived client-side. `sharedContent.addToLibrary({entryId})` creates a session on the entry's source+track inside one transaction with a `FOR SHARE` liveness lock — a removed/unshared entry can no longer be added, retained UUIDs included, while `alreadyExisted` idempotence and the guest library cap work exactly like `studySessions.create` (adds **count** against the guest cap). A missing CEFR pref for the entry's language returns a typed 422 `CEFR_REQUIRED` so the client runs the CEFR dialog first.
- **Authorization hardening that ships with the catalog**: `studySessions.create` only attaches to movie/TV (global types), sources the caller owns, or tracks with a live entry; `textTracks.importFromPaste` refuses foreign personal sources. Before the catalog, possession of a personal source/track UUID implied ownership — the catalog hands those UUIDs to strangers, so possession no longer grants access.
- **Share state surface**: `sharedContent.getShareState`/`setShared` (owner-only; non-owners, non-shareable types, guests, and tombstones all read `not-shareable`, so recipients of shared content never see an owner toggle on their copy).
- **Web surfaces**:
  - **Explore** (`/explore`) — an overflow tab view (own desktop sidebar entry with a compass icon between Sessions and Practice; on mobile the Dashboard tab stays highlighted, `OverflowTabHeader backTo='/dashboard'`). Header + "Shared content from the community" subtitle, then a language `FilterChip` row (All + one chip per language present in the feed, localized names) and the entry list. The chip preselects the user's `lastTargetLanguage` when the feed has it; an explicit pick persists as `?lang=` (so the dashboard section could deep-link it), stale codes degrade to All. Filtering is client-side over the one ≤100-entry feed read. Entries render through the same unified `MediaListItem` as the Sessions list (stacked 16:9 media card on mobile, thumb-left row on desktop) with a trailing chevron instead of a ⋮ — shared entries have no user-scoped actions; the detail screen owns them. YouTube entries use the `i.ytimg.com` thumbnail, everything else the procedural letterform tile (`LetterformTile` — see SPEC.md Sessions list); articles show their source domain in the meta line. Tapping navigates to the entry's **detail screen**. No difficulty, no attribution, no date.
  - **Detail screen** (`/explore/$entryId`, full-view modal screen with a chevron back to the actual opener, `/explore` on deep links) — the read-before-you-add preview: a landscape 16:9 thumbnail + title + meta header, a client-computed `~N words · M lines` stat line (`Intl.Segmenter`, whitespace-split fallback for invalid tags), and the track's full text rendered read-only (no glossing, no selection commit). A sticky bottom **`Start reading`** CTA runs the add: it creates the session and navigates into the reader with `replace: true` (`alreadyExisted` → the "picking up where you left off" toast; back from the reader returns to the catalog, not the consumed detail screen). A 404 (dead deep link, or the entry dying between listing and open) renders a "no longer shared" state — expected, not error-tracked (`expectedNotFound` query meta); other failures render a retry state. If the entry dies between the detail fetch and the CTA tap, the add's 404 refetches the detail so the view flips to the not-found state instead of a CTA that does nothing.
  - **Add + CEFR** — if the user has no CEFR pref for the entry's language, the CTA first opens the standalone `CefrPromptDialog` (rebuilt around the shared `CefrStep`, `ResponsiveOverlay`), saves via the normal `setCefrForLanguage`, then adds. For a guest this doubles as the "what are you learning?" moment — no separate onboarding step. Guest cap 403s open the existing save-progress overlay.
  - **Dashboard rails** ("Featured content" above Continue learning, "From the community" below it) — two single-row horizontal rails for **everyone**, not just new users, both built on the shared `MediaRail` chrome and the unified vertical media card (`ExploreMediaCard`: 16:9 media flush on top, 2-line clamped title, meta line; ~230px cards on mobile, exactly four per view on desktop) + "Explore all →"; cards navigate to the same detail screen. One computation feeds both (`useDashboardExploreEntries`): entries already in the viewer's library (`inLibrary`) are hidden from both, and once the user has any target language (session languages ∪ `lastTargetLanguage`) both narrow to those languages — with none (brand-new users/guests) they show everything; no chip row, the full multi-language catalog stays on /explore. **Featured** takes up to 12 featured entries; **From the community** takes the newest 12 of everything else (dedup against the rail by entry id), so the two sections never repeat each other. Each rail scroll-snaps with a hidden scrollbar (mobile: full-bleed edge-to-edge with `scroll-pl` matching the page inset; swipe only); desktop adds paging chevrons that fade in on rail hover (or keyboard focus) and render only in a direction that can still scroll, plus background-fade edge gradients on scrollable sides (mobile too) — the Continue-learning section reuses the same `MediaRail` with a `md:grid` scroller override, whose lack of horizontal overflow hides the chevrons/fades on desktop. Each section renders skeletons while the sessions/feed queries load and renders nothing once they prove it empty. Removing a session invalidates the catalog feed too — a removal auto-unshares its entry server-side, and the dashboard hosts all these surfaces at once.
  - **Admin moderation, inline in Explore** (test users only; `checkIsTestUser` cosmetically, `assertTestUser` authoritatively; strings untranslated like all admin surfaces) — an extra status chip row (`live` / `unshared` / `removed`) above the language chips: `live` reads the **public** feed (moderating means seeing exactly what users see), the non-live chips read the server-filtered `adminList`. The status is URL-backed (`?status=`) because the detail screen is a separate route — component state would reset to Live on every back-navigation while working through a list; a non-admin with a crafted URL sees nothing (query enabled-gated, backend 403s). The detail screen adds an **admin panel** between header and text: status (+ removal reason on tombstones), Feature/Unfeature (live only), and Remove with an inline required-reason input; the Start-reading CTA hides on non-live entries (adding would 404). The former shared-content card in admin settings is retired — Explore is the single moderation surface.
  - **Owner share toggle** — the session ⋮ actions overlay shows a Share-to-Explore / Unshare row driven by `getShareState` (fetched when the overlay opens; hidden entirely on `not-shareable`). Copy warns against sharing private/copyrighted content and that unsharing leaves others' copies intact.
  - **Paste-wizard checkbox** — "Share to Explore" (off by default, hidden for guests) on the paste step; rides `studySessions.create` as `shareToExplore`.

## During a session

- No in-movie sync. The app is for triage and lookup, not playback.
- The mid-source screen is a search bar over the track plus a scrollable list of segments. Movie segments show a timestamp; text segments don't. That is the entire mid-source UI. Search filters **client-side** over the already-loaded track (no server round-trip, no result cap; results keep document order) using the shared fuzzy matcher (`@flicktionary/core/utils/search-match`): case-, accent- and joiner-punctuation-insensitive ("pantywaist" finds "panty-waist"), full-phrase substring first (which also gives prefix matching and covers scripts without word boundaries), then per-query-word AND with one edit of typo tolerance for words of 4+ letters ("vixin" finds "vixen"). Meaning-bearing marks (Japanese dakuten, Indic viramas/vowel signs) are never stripped. There is no stemming — searching an inflection finds that inflection (or anything within a typo/prefix of it), not its other conjugations.
