...
- **Web surfaces**:
  ...
  - **Dashboard featured rail** ("Featured content", directly above Recent) — a single-row horizontal rail for **everyone**, not just new users: up to 12 featured entries as vertical cards (`ExploreRailCard`: fixed-width, `aspect-video` thumb on top, 2-line clamped title) + "Explore all →"; cards navigate to the same detail screen. Entries already in the viewer's library (`inLibrary`) are hidden, and once the user has any target language (session languages ∪ `lastTargetLanguage`) the rail filters to those languages — with none (brand-new users/guests) it shows all featured entries; no chip row, the full multi-language catalog stays on /explore. The rail scroll-snaps with a hidden scrollbar (mobile: full-bleed edge-to-edge with `scroll-pl` matching the page inset; swipe only); desktop adds paging chevrons that fade in on rail hover (or keyboard focus) and render only in a direction that can still scroll, plus background-fade edge gradients on scrollable sides (mobile too). The section renders skeletons while the sessions/feed queries load and renders nothing once they prove the rail empty (no featured entries in the user's languages, or everything already added). Removing a session invalidates the catalog feed too — a removal auto-unshares its entry server-side, and the dashboard hosts both surfaces at once.
