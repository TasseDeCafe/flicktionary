...
- User content is private by default and RLS-scoped. The **one deliberate exception** is the shared-content catalog below: publishing exposes the source title publicly and lets other users create sessions on the track (which grants them segment-text reads through the normal session-scoped path). Nothing else about a source ever leaves its owner — raw source URLs in particular are never public (an extension-imported URL is the verbatim tab URL and can carry signed tokens; only the server-parsed domain is exposed).

## Shared content (Explore)

Users' own content can be published into a public, cross-user catalog (`shared_content_entries`, one row per published text track) that new and anonymous users browse and add from in one tap. Backend: `apps/backend/src/service/shared-content/` (policy, publish service, add-to-library) + `shared-content-router`.
