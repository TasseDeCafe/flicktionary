...
4. Analytics: guests are identified as usual with an `is_guest` person property so drive-by accounts can be filtered from funnels.
5. **Guest → account conversion.** A persistent (non-dismissible) "Save your progress" banner on the dashboard and the More tab, plus an Account-page row, lead to `/save-progress`, which converts the anonymous account **keeping the same user id** — all sessions, vocabulary, and SRS state carry over. Email path: `updateUser({ email })` sends a confirmation link (the `email_change` template) to `/login/email/verify?type=email_change`; if the server runs with email confirmations disabled, the change applies instantly and the UI goes straight to the success state. Google path: `linkIdentity` (requires Supabase's manual-linking setting), returning to `/save-progress?linked=google`. Either way the session is refreshed afterwards so the `is_anonymous` JWT claim drops and guest restrictions lift; the backend reads the converted user's email from the verified top-level claim (`user_metadata` is never written by conversion). If the email or Google identity already belongs to another account, Supabase refuses and the screen offers "log in instead" — accounts are never merged. The normal login flow is never offered to a guest: signing in there mints a different user and would orphan everything the guest saved.
6. **Guest source cap.** A guest's library is capped at `MAX_SOURCES_PER_GUEST` (default 3) non-adhoc content sources — sources they created plus globally-deduped movie/TV sources they attached via a session. Enforced server-side at every source-creating path (web wizards, extension ingest, lesson import, Telegram) and at session creation, where deduped sources actually enter a guest's library (`assertGuestSourceQuota` / `assertGuestSessionQuota` in `apps/backend/src/transport/database/guests/guest-source-quota.ts`). The guard reads the live `auth.users.is_anonymous` column rather than the JWT claim, so JWT-less surfaces (Telegram bot) are covered and conversion un-caps immediately, before the token refreshes. Never blocked: re-attaching a source already in the library, idempotent re-ingests, and adhoc word saves; deleting a session frees the slot of a source the guest didn't create. Over-cap requests answer `403` with code `GUEST_SOURCE_LIMIT_REACHED`; the web app maps it to a create-account overlay leading to `/save-progress` (and captures `guest_source_limit_reached`), the Telegram bot replies with the same create-account pitch.
