...
- Sticky footer: `Practice these terms` button (full-width on mobile,
  right-aligned on desktop) that launches the **exercise-first warm-up** for
  this session's new terms (`/practice/warmup/$targetLanguage`, see "Strengthen
  exercises + leech rehab + warm-up") rather than the flashcard reviewer —
  general flashcard practice stays reachable from the Practice tab. Disabled when
  no cards are kept (effectively always enabled once any term has data).
...
- **Session modes (generated-text reading).** A practice session is scoped to one target language, one start mode, and one pool: `review_due` snapshots only already-introduced due terms in the passive pool; `learn_new` snapshots unseen passive terms up to the remaining per-day new-term allowance; `learn_extra` intentionally bypasses the daily new-term cap for users who choose to keep going; `mixed` snapshots due terms plus unseen passive terms up to the remaining daily new-term allowance; `active_drill` snapshots only terms with an enabled `(meaning_production, '')` facet — all currently-due active terms and all unseen active terms with **no daily new-term cap** (the cap is a passive-pool concept, intentionally not inherited so active drills never eat the passive new-term allowance). The one-active-session-per-(language, pool) rule still wins: starting while an active session exists for the same pool resumes that session. (Accepted transitional inconsistency: reading `mixed` still introduces new terms directly into FSRS via implicit ratings, a second intro path beside the composed queue's exercise-first on-ramp; reading lives under Custom practice.)
...
- Routes: `/practice/strengthen/$targetLanguage` (leeches + bonus; Zod search: `pool`, optional `sessionHard` userLookupId array — carried in the URL so the list survives refresh) and `/practice/warmup/$targetLanguage` (session-scoped warm-up; search `studySessionId`) render the shared `ExerciseSessionView`, differing only in fetch source and copy (`copyVariant: 'rehab' | 'warmup'`); day-to-day gate serving happens inside the composed Practice queue itself. Strengthen's remaining entry point is the post-session CTA (primary `Strengthen` button on the composed completion screen when the session produced again/hard terms; the back button is the skip path — reading completion is unchanged though its ratings still warm bonus banks); `startStrengthenSession` requests `parkedOrigin: 'leech'` for its gate track. Warm-up's dedicated entry point is the session-vocabulary footer (parks both pools); the old language-wide `warmup-continue` resume and the landing's leech-parked affordance were absorbed by the composed queue.
...
3. Sticky-footer `Practice these terms` button launches the exercise-first warm-up for this session's new terms (it parks them into scaffolding and serves gate exercises; after graduation they enter the flashcard queue). General flashcard practice over the language-wide pool stays on the Practice tab — kept chunks feed into it via the user-lookups upsert that fires on the auto-keep transition.
