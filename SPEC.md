...
(and the stale-deep-link fallback) lands on `/dashboard`. A `More stats`
link on the card opens `/stats`.
...
- **Sort & filter control.** Search input plus a single `Sort & filter` button (a Radix popover on desktop, a bottom sheet on mobile; the popover caps to the viewport and scrolls internally; the trigger carries a dot when any filter is active). All sort/filter state persists in the URL so reload/deep-link survives (`?lang=&sort=&status=&skills=&forms=`; stale tokens — including the retired `?mode=` — degrade to defaults via `.catch`). `lang` selects the target language (the practice landing deep-links here); it is validated against the user's actual languages (a stale code falls back to the saved/first language) and preserved by every filter write. Contents:
...
- **More tab** consolidates user prefs and account pages: a sectioned list (General / Settings / About) with sub-pages for Account and Languages, plus an inline `Switch` row for `LLM-suggested terms`. About opens with a `User guide` row linking to `/user-guide`.
