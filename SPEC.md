...
3. **Per-chunk Full exploration (deferred, on-demand)** — one call per card,
   ...
   - Output: refined basic columns (the model may revise them based on
     ...
     notes, bolded context segment). Per-chunk L1 notes (e.g. an English
     speaker's confusion between `près de moi` and `chez moi`) are
     generated by the model from its own training knowledge of L1→L2
...
Per-card chat seed prompt = methodology + `(L1, target, CEFR)` + source context blob (cached) + chunk + 10 surrounding segments + the card's current basic data + extras (if populated, including any per-chunk L1 notes). The user's question is the only dynamic turn.
