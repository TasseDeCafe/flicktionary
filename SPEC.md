...
The extension has its own spec — behavior, architecture, fork lineage, and the donor-model policy for harvesting from upstream: **`packages/extension/EXTENSION-SPEC.md`** (the source of truth for the extension; what follows here is the backend-coupling summary).
