...
6. Everything else email-shaped degrades gracefully for guests: account deletion works (the `removals` audit row records a NULL email), the contact form simply starts with an empty (editable) email field, and admin/test-user gating never matches an empty email.
