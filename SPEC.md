...
- **Sessions list** (`/sessions`, its own desktop sidebar entry; on mobile an overflow tab view under `Dashboard` — see Mobile above) is the full library. Once any session exists, a search input (250 ms-debounced title match, transient local state — not in the URL) and a filter panel (`SlidersHorizontal` trigger; popover on desktop, drawer on mobile) with language pills + newest/oldest sort sit above the list; language/sort/type persist in the URL (`?lang=` / `?sort=` / `?type=`, defaults dropped so URLs stay clean; a stale `?lang=` degrades to all), filters apply **before** TV-show grouping so groups reflect them, and an `N sessions` count line sits above the rows. The zero-sessions **empty state** is a first-run block (what a session is + the primary `+ New` actions + an extension pointer) instead of a bare "no sessions" line. Sessions list offers `All / Movies / TV / Texts / Articles / YouTube / Streaming / Lessons` filter chips (label-only — no counts) so the unified list stays scannable as content types diversify. Rows are the unified media list item (`MediaListItem`): a stacked card on mobile (16:9 media flush on top, the relative date joining the meta line, ⋮ in the meta row's corner) and a thumb-left row on desktop (172px 16:9 media flush to the card's left edge, the relative date on the right edge — `Today` / `Yesterday` / short date, year only when it differs). The media is the TV episode still, the movie/show TMDB backdrop (w780, snapshotted into `content_source.metadata` at wizard time; `db:backfill-backdrops` re-queries TMDB for pre-existing rows), or the YouTube thumbnail built client-side from `youtubeVideoId`. Meta shows year + language + the difficulty stat — the session's stored CEFR level was dropped (it's the user's level, which read as content difficulty; the difficulty stat is the honest signal). Sources without any image (texts, articles, lessons…) get the procedural **letterform tile** (`LetterformTile`): the title's first letter as an oversized serif glyph with a lighter offset copy and a small outlined echo, on a grainy background tinted per content type, deterministic per title, composed 16:9 and sliced object-cover-style so any tile aspect crops the same art. Synthetic adhoc sessions (the per-(user, language) "Personal vocabulary" pseudo-sessions backing the Add-a-word flow) are filtered out at the query layer — they never appear under any chip. Each row carries a **⋮ options menu** opening a session actions overlay (sheet on mobile, dialog on desktop; `session-actions-overlay.tsx`) with the owner's Share-to-Explore/Unshare toggle when the track is shareable (see `docs/READER-SPEC.md` "Shared content (Explore)") and whose destructive `Remove session` row opens the confirmation dialog; confirming soft-deletes the session via `study_session.deleted_at` — the session disappears from the list, but the kept cards stay in the user's vocabulary and the source text is retained so future "my vocabulary" views can back-link to it. The confirmation overlay is explicit about this and points users at account deletion for full erasure. The same ⋮ menu sits in the reader header; removing from there navigates back to the list.
- **Explore** (`/explore`, its own desktop sidebar entry; on mobile an overflow tab view under `Dashboard`) is the community shared-content catalog: language chips over entry cards that open a full-text detail screen with an explicit `Start reading` add CTA. Deep behavior — share modes, moderation gating, publish lifecycle, the detail/add flow — lives in `docs/READER-SPEC.md` "Shared content (Explore)".
  - **TV sessions group by show.** Episodes are one `study_session` each, but the list collapses every TV episode of the same show into a single tappable **show row** (show backdrop + show title + `<lang> · N episodes`, latest-episode date, chevron — no ⋮, a group has no single session), derived client-side from the session list (`deriveTvShows`, keyed on the `tmdbShowId` now carried on the session DTO from `content_source.metadata`) — no per-episode rows clutter the list. Movies and every other source type stay individual rows; show rows and loose rows interleave by recency. Tapping a show row opens the **show detail screen** (a modal drill-in): a scrollable list of its added episodes (`S0xE0y · <episode>`, each with the episode's TMDB still — show backdrop when TMDB has none — each linking to its session, each with the same soft-delete trash control) under a **sticky-footer `Add episode`** button. `Add episode` deep-links into the new-session wizard pre-seeded for this show (see "Start a movie or TV session"). The detail screen reads the cached session list, so it opens with no extra fetch; removing the last episode collapses the show and falls back to the Sessions list.
