...
This means there's exactly one copy of each migration file on disk; the four envs cannot drift. The app is deployed, so migrations are append-only: do not edit an existing migration to change the database schema unless the user explicitly asks for a history rewrite before that migration has been applied anywhere. Every schema/data migration change should be a new migration. The workflow is:
