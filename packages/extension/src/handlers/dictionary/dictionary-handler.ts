import { Command, DictionaryImportRecordLocalBulkMessage, Message } from '@asbplayer-fork/common'
  ...
  handle(command: Command<Message>, sender: Browser.runtime.MessageSender, sendResponse: (response?: any) => void) {
    switch (command.message.command) {
      case 'dictionary-get-bulk': {
        ...
        this.dictionaryDB.getBulk(message.profile, message.track, message.tokens).then((result) => sendResponse(result))
        return true
      }
      ...
      case 'dictionary-delete-record-local-bulk': {
        ...
        this.dictionaryDB.deleteRecordLocalBulk(message.profile, message.tokens).then((result) => sendResponse(result))
        return true
      }
