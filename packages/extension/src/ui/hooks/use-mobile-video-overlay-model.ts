...
export const useMobileVideoOverlayModel = ({ location }: Params) => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        console.log(
          'Failed to request overlay model, retrying in 1s. Message: ' + (e instanceof Error ? e.message : String(e))
        )
  ...
  useEffect(() => {
    ...
    const listener = (message: any, sender: Browser.runtime.MessageSender, sendResponse?: (message: any) => void) => {
