...
export default class WordInteractionController {
  ...
  private _checkAndClearStaleUI() {
    ...
    if (this.tooltipWordElement) {
      ...
      const isStale =
        !this.tooltipWordElement.isConnected ||
        !container ||
        !document.body.contains(container) ||
  ...
  private _handleMouseEnter(e: Event) {
    ...
    this.hoverTimeout = setTimeout(() => {
      ...
      if (this.selectionState.selectedWords.length > 1 && this.selectionState.selectedWords.includes(target)) {
  ...
  private _handleMouseOver(e: Event) {
    const node = e.target
    const word = node instanceof Element ? (node.closest('.asbplayer-word') as HTMLElement | null) : null
    ...
    const overSelectedWord =
      this.selectionState.selectedWords.length > 1 && word !== null && this.selectionState.selectedWords.includes(word)
