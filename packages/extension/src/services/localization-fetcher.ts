...
import { SettingsProvider, supportedLanguages as defaultSupportedLanguages } from '@asbplayer-fork/common/settings'
...
export const fetchLocalization = async (lang: string): Promise<Localization> => {
  ...
  return (
    (await cachedStringsForLang(lang)) ?? (await bundledStringsForLang(lang)) ?? (await bundledStringsForLang('en'))!
  )
}
...
const bundledStringsForLang = async (lang: string): Promise<Localization | undefined> => {
  for (const defaultLang of defaultSupportedLanguages) {
    if (lang === defaultLang) {
      return {
        lang,
        strings: await (await fetch(browser.runtime.getURL(`/asbplayer-locales/${lang}.json` as PublicPath))).json(),
      }
    }
  }
