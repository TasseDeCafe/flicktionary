...
7. **Open the PR** with `gh pr create --base main`. Write the title in the same conventional-commit form as the commits' primary change. Body structure (match the house style — see any recent merged PR):
...
8. **Report** the PR URL and stop. Do not merge. If the user later says to merge, confirm CI/checks first.
