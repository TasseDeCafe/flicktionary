...
// normalizer (NFC -> strip U+0301 -> trim -> lower; NFC first so orthographic
// acutes in decomposed input survive) is the byte-for-byte twin of
...
const resolveFacetSources = async (
  ...
  const rows = (await sql`
    SELECT
      lower(trim(regexp_replace(normalize(c.surface_form, NFC), chr(769), '', 'g'))) AS norm_form,
      lower(trim(regexp_replace(normalize(ul.headword, NFC), chr(769), '', 'g'))) AS norm_headword,
      c.study_session_id,
      c.segment_id,
      s.text_track_id,
      ts.text AS sentence
...
// Opus on enable. The SQL key normalizer (NFC -> strip U+0301 -> trim -> lower;
// NFC first so orthographic acutes in decomposed input survive) is the
// byte-for-byte twin of normalizeTargetForm() in @flicktionary/core
