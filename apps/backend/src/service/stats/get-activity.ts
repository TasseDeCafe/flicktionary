...
// One fetch serves both surfaces: the dashboard card shows the last 7 days,
// the stats view all 14.
...
export const getActivity = async (params: { userId: string }, deps: StatsDependencies): Promise<ActivityData> => {
  const [today, introduced, markedKnown, activeDays] = await Promise.all([
  ...
  return { days, perLanguage, streakDays: computeStreakDays(activeDays, today) }
}
