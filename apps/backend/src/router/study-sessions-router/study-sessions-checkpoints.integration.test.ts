...
describe('study-sessions checkpoints', () => {
  ...
  test('collect returns UNSUPPORTED_LANGUAGE for a non-kaikki language and preview reports unsupported', async () => {
    ...
    await UserTargetLanguagePrefsRepository().upsertCefr(userId, 'ja', 'B1')
    const jaSession = await createReadingSession(userId, 'ja')

    const preview = await request(testApp)
      .get(`/api/v1/study-sessions/${jaSession.id}/checkpoint-preview?toSegmentIndex=50`)
    ...
    const collected = await request(testApp)
      .post(`/api/v1/study-sessions/${jaSession.id}/checkpoints`)
      .set(buildAuthorizationHeaders(token))
