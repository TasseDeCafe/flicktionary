...
# Use jq to check if @flicktionary/backend is in the packages.items array
if echo "$output" | jq -e '.packages.items[] | select(.name == "@flicktionary/backend")' > /dev/null; then
