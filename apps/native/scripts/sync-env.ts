...
const cleanupExistingMount = (): void => {
  try {
    ...
    execSync('doppler run --project native --config dev_personal clean', { stdio: 'pipe' })
  } catch {
...
const getDopplerSecrets = (): DopplerSecrets => {
  try {
    const output = execSync('doppler secrets --json --project native --config prd', { encoding: 'utf-8' })
...
const syncEnvironmentVariables = (environment: string) => {
  ...
  if (publicKeys.length > 0) {
    ...
    execSync(
      `EAS_NON_INTERACTIVE=1 doppler run --project native --config dev_personal -- eas env:push ${environment} --path ${ENV_FILE_PATH}`,
      {
        stdio: 'inherit',
  ...
  for (const key of secretKeys) {
    ...
    if (easVariableNames.has(key)) {
      ...
      execSync(
        `EAS_NON_INTERACTIVE=1 doppler run --project native --config dev_personal -- eas env:update ${environment} --variable-environment ${environment} --variable-name ${key} --value "${value}" --visibility secret --non-interactive`,
        { stdio: 'inherit' }
      )
    } else {
      console.log(`Creating secret variable ${key}...`)
      execSync(
        `EAS_NON_INTERACTIVE=1 doppler run --project native --config dev_personal -- eas env:create ${environment} --name ${key} --value "${value}" --visibility secret --non-interactive`,
        { stdio: 'inherit' }
      )
    }
  }
