...
import { VideoData } from '@asbplayer-fork/common'
...
export default defineUnlistedScript(() => {
    ...
    for (const arg of args) {
      ...
      if (selectedSubUrlMatch) {
        ...
        if (plexSessionIdMatch) {
          selectedSubUrl = selectedSubUrl.replace(plexSessionIdMatch[0], `&X-Plex-Session-Id=${plexSessionId}`)
        ...
        if (plexPlaybackIdMatch) {
          selectedSubUrl = selectedSubUrl.replace(plexPlaybackIdMatch[0], `&X-Plex-Playback-Id=${plexPlaybackId}`)
  ...
  document.addEventListener(
    ...
    async () => {
      ...
      const xmlText = await (await fetch(`${serverUrl}/library/metadata/${ratingKey}?X-Plex-Token=${plexToken}`)).text()
