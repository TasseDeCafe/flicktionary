...
export default defineUnlistedScript(() => {
  setTimeout(() => {
    ...
    function timedTextUrls(): Map<string, string> {
      ...
      const root =
        netflix?.appContext?.state?.playerApp?.getState?.()?.videoPlayer?.cadmiumPlayerRepository?.playersById?.[
          activeSessionId
        ]
      ...
      // Untyped minified Netflix player state — probed structurally below.
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const dataForTrack = (
      ...
      urlsByTrackId?: Map<string, string>
    ...
    const buildResponse = async () => {
      ...
      response.subtitles = (np.getTimedTextTrackList() ?? [])
        .map((track: NetflixTimedTextTrack) => dataForTrack(track, urlsByTrackId))
    ...
    const fetchDataForLanguage = async (e: Event) => {
      ...
      try {
        ...
        const track = np
          ...
          ?.find((track: NetflixTimedTextTrack) => dataForTrack(track)?.language === language)
