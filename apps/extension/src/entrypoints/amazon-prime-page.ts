import { VideoData, VideoDataSubtitleTrack, VideoDataSubtitleTrackDef } from '@asbplayer-fork/common'
      ...
      try {
        if (lastEntityId) {
          ...
          const capturedUrls = await poll(() => {
            const urls = metadataUrls[entityId]
            return Boolean(
              urls && urls.playerChromeResourcesUrl && urls.vodPlaybackResourcesUrl && urls.vodPlaybackResourceBody
            )
          })
          ...
          if (capturedUrls) {
            ...
            document.dispatchEvent(
              new CustomEvent('asbplayer-synced-data', {
                detail: {
                  error:
                    'Could not detect subtitles. Try refreshing this page and starting playback. (incomplete metadata)',
                  basename: '',
                  subtitles: [],
                },
              })
          ...
          document.dispatchEvent(
            new CustomEvent('asbplayer-synced-data', {
              detail: {
                error:
                  'Could not detect subtitles. Try refreshing this page and starting playback. (no video ID captured)',
                basename: '',
                subtitles: [],
              },
            })
