...
export default class VideoDataSyncController {
  ...
  private async _syncData(data: VideoDataSubtitleTrack[], userRequested = false) {
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        const { extension, url, language, localFile, isAutoGenerated } = data[i]
  ...
  private async _syncDataArray(data: ConfirmedVideoDataSubtitleTrack[], syncWithAsbplayerId?: string) {
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        const { name, language, extension, url, localFile, isAutoGenerated } = data[i]
        const subtitleFiles = await this._subtitlesForUrl(name, language, extension, url, localFile, isAutoGenerated)
        if (subtitleFiles !== undefined) {
          subtitles.push(...subtitleFiles)
        }
      }
  ...
  private async _subtitlesForUrl(
    ...
    // Auto-generated (ASR) tracks carry a filename marker so the subtitle
    // reader knows re-chunking is allowed — the payload alone can't tell them
    // apart from human-authored karaoke tracks, which are word-timed too but
    // whose cue structure must be preserved.
    const fileExtension = isAutoGenerated === true && extension === 'ytsrv3' ? 'asr.ytsrv3' : extension
