...
export const DangerZoneView = () => {
  ...
  return (
    <ModalScreen onClose={() => navigate({ to: '/more' })} closeIcon='chevron' title={t`Danger Zone`}>
      <main className='bg-destructive/5 flex-1 overflow-y-auto p-4'>
        <div className='mx-auto flex w-full max-w-2xl flex-col gap-6'>
          <div className='px-2'>
            <h2 className='text-destructive mb-2 text-lg font-medium'>{t`Warning`}</h2>
            <p className='text-destructive'>{t`Actions in this section can lead to permanent data loss.`}</p>
          </div>

          <Card className='border-destructive/30 bg-card'>
            <CardHeader className='pb-0'>
              <CardTitle className='text-destructive text-base'>{t`Delete my account`}</CardTitle>
