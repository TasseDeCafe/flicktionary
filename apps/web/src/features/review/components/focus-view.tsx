  ...
  if (isLoading) {
    return (
      <ModalScreen onClose={closeToTriage} closeIcon='chevron' title={t`Card`}>
        <div className='mx-auto max-w-4xl px-4 py-6 text-sm text-muted-foreground'>{t`Loading card…`}</div>
      </ModalScreen>
    )
  }
  if (!card) {
    return (
      <ModalScreen onClose={closeToTriage} closeIcon='chevron' title={t`Card`}>
        <div className='mx-auto max-w-4xl px-4 py-6 text-sm text-muted-foreground'>{t`Card not found.`}</div>
      </ModalScreen>
    )
  }
        ...
        <ModalScreen onClose={closeToTriage} closeIcon='chevron' title={title} rightSlot={headerNav}>
          <div className='flex-1 overflow-y-auto px-4 py-4'>
            <div className='mx-auto flex max-w-4xl flex-col gap-6'>
              <section>
                <h2 className='mb-3 text-sm font-semibold tracking-wide text-muted-foreground uppercase'>{t`Card`}</h2>
              ...
              <section>
                ...
                <h2 className='mb-3 text-sm font-semibold tracking-wide text-muted-foreground uppercase'>{t`Full exploration`}</h2>
                {hasExtras ? (
